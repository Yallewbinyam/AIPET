...
| PLB-9 | Capability 13 Day 3 needs Windows live verification on real Windows VM (install + service start + reboot survival + watchdog revocation detection + uninstall). Code complete (51 cross-platform tests passing, Linux .deb still works), but the runtime path on Windows has only been mock-tested. Blocks Windows-shop hospital pilots — biomedical engineering departments running Windows cannot be onboarded until this is exercised end-to-end. | Day 3 (deferred at end of session due to fatigue) | ~1 hour focused on a real Windows VM | Open | Before first Windows pilot customer |
...
**Current status:** Capabilities 1–12 ✅ Complete + Capability 13 Day 1 ✅ + Capability 13 Day 2 ✅ + Capability 13 Day 3 🟡 (code complete, Windows live verify pending PLB-9). ~13.5 of 33 capabilities. 19.5 remaining (13 Day 3 live verify, 14–32).
...
| 13 Day 3 | Windows service + Windows installer | 🟡 **CODE COMPLETE — Windows live verify pending (PLB-9)**. Platform-portable `aipet_agent.py` v1.2.0 (added `IS_WINDOWS`/`IS_LINUX` constants, `_resolve_config_path/_log_dir/_install_dir` helpers using `pathlib.PureWindowsPath`/`PurePosixPath`, replaced Linux-only `subprocess(["ip", …])` with `psutil.net_if_addrs()`, guarded `os.fork()` behind `if IS_WINDOWS: log warning else: fork`). Watchdog already cross-platform (no changes needed). NSSM 2.24 win64 bundled at `agent/packaging/windows/nssm.exe` (~331 KB, public domain, license shipped). `install_windows.bat`: 3-question UX, admin check (`net session`), Python 3.8+ detection, nmap detection, regex-validates `aipet_…` key, copies to `%ProgramFiles%\AIPET\`, writes config summary to `%ProgramData%\AIPET\agent.conf` (key value held in NSSM env, NOT on disk), calls `aipet-agent-service-install.bat` (NSSM service install with **`AppExit 1 Stop`** for revocation security, `AppExit Default Restart` for crash recovery, `SERVICE_AUTO_START`, log rotation 10 MB, all 8 `AIPET_*` env vars), registers Add/Remove Programs entry, self-tests cloud reachability + first telemetry. `uninstall_windows.bat`: stops service, rmdirs install + ProgramData, deletes registry key. `agent/packaging/aipet-agent-windows-1.0.0.zip` (139 KB) is the deployable bundle. **51 new tests** (cross-platform path resolution under mocked `platform.system()`, no-fork-on-Windows static + runtime asserts, watchdog cross-platform, packaging artefact lints incl. NSSM PE32+ binary validation, `AppExit 1 Stop` security assertion, install.bat structural lints, Linux .deb backward-compat). 447 backend tests / 3 skipped. Linux .deb still builds and installs identically. **What is NOT yet verified**: actually running the installer on a real Windows host, sc-querying AipetAgent, surviving a reboot, observing the watchdog's exit-code-1 → STOPPED transition, and the registry/AppWiz cleanup on uninstall. See PLB-9. |
